...
class DefaultTrainer(BaseTrainer):
    ...
    def __init__(self, config, model, loss, metrics, optimizer, lr_scheduler, resume, data_loader,
        ...
        self._load_pretrained_weights(**extra_args)  # for loading the pretrained weights of R_refine_model

    def _load_pretrained_weights(self, **extra_args):
        R_refine_model_checkpoint_path = extra_args.get('R_refine_model_checkpoint_path')
        if R_refine_model_checkpoint_path:
            R_refine_model_checkpoint = torch.load(R_refine_model_checkpoint_path)
            if self.data_parallel:
                self.model.module.R_refine_model.load_state_dict(R_refine_model_checkpoint['model'])
            else:
                self.model.R_refine_model.load_state_dict(R_refine_model_checkpoint['model'])
            print('load R_refine_model_checkpoint ...')
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, sample in enumerate(self.data_loader):
            ...
            # get network output
            # (N, C, H, W) GPU tensor
            # A_infinity_pred, R_pred = self.model(I_alpha, I, T)
            A_infinity_pred, R_pred = self.model(I_alpha, I, T)
            ...
            with torch.no_grad():
                if batch_idx % 100 == 0:
                    ...
                    self.writer.add_image('I', make_grid(I))
            ...
            model_loss = self.loss(A_infinity_pred, A_infinity, R_pred, R)
            model_loss.backward()
            self.optimizer.step()
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, sample in enumerate(self.valid_data_loader):
                ...
                loss = self.loss(A_infinity_pred, A_infinity, R_pred, R)

                # calculate total loss/metrics and add scalar to tensorboardX
                self.writer.add_scalar('loss', loss.item())
                total_val_loss += loss.item()
                total_val_metrics += self._eval_metrics(R_pred, R)

...
from .dataset import dataset_R_refine_model
...
class TrainDataLoader(BaseDataLoader):
    def __init__(self, data_dir, extra_dir, batch_size, shuffle, validation_split, num_workers):
        transform = None
        self.dataset = dataset_R_refine_model.TrainDataset(data_dir, extra_dir, transform=transform)
...
class InferDataLoader(DataLoader):
    def __init__(self, data_dir, extra_dir):
        transform = None
        self.dataset = dataset_R_refine_model.InferDataset(data_dir, extra_dir, transform=transform)
